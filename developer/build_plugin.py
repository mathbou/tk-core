...
from tank.util import filesystem, sgre
...
def _validate_manifest(source_path):
    ...
    if sgre.search(r"^[a-zA-Z0-9_\.]+$", manifest_data["plugin_id"]) is None:
        raise TankError(
...
def _bake_manifest(manifest_data, config_uri, core_descriptor, plugin_root):
    ...
    module_name = "sgtk_plugin_%s" % sgre.sub(r"\W", "_", manifest_data["plugin_id"])
